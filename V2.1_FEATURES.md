...
- **Error Messages**:
  - `⚠️ Please enter a valid Forecast ID` - if Forecast ID is missing
  - `⚠️ Please provide the realised price sequence for this forecast horizon` - if actual prices are missing
  - `⚠️ Forecast ID 'XXX' not found. Please load the forecast first.` - if Forecast ID doesn't exist
...
- **User Feedback**:
  - `✅ Successfully loaded N actual prices for SYMBOL` - when data is available
  - `ℹ️ Forecast period has not elapsed yet. X days remaining.` - when forecast is still active
  - `ℹ️ Please input actual prices manually below when they become available.` - when data isn't ready
...
- Warning messages with warning icons (⚠️)
- Info messages with info icons (ℹ️)
...
1. **Load Forecast Button**:
   ```
   User clicks → Check Forecast ID → Load snapshot → 
   If found: Show details + Fetch actuals → 
...
2. **Evaluate Button**:
   ```
   User clicks → Validate Forecast ID → Validate actual prices → 
   Parse prices → Load forecast → Evaluate → Show results
   ```
...
4. If now < forecast_end_date:
     → Return "not available yet" message
5. Else:
     → Fetch historical prices from yfinance
     → Extract prices for forecast period
     → Return prices + success message
```
...
1. **User enters Forecast ID** and clicks "Load Forecast for Evaluation"
   - If ID is empty: `⚠️ Please enter a valid Forecast ID`
   - If ID not found: `⚠️ Forecast ID 'XXX' not found`
   - If found: `✅ Loaded forecast for AAPL` + attempts to fetch actuals

2. **If forecast period has elapsed**:
   - System automatically fetches actual prices
   - Input field is auto-populated
   - User sees: `✅ Successfully loaded 30 actual prices for AAPL`

3. **If forecast period hasn't elapsed**:
   - User sees: `ℹ️ Forecast period has not elapsed yet. 15 days remaining.`
   - Input field remains empty for manual entry later

4. **User clicks "Evaluate Forecast Accuracy"**:
   - If Forecast ID missing: `⚠️ Please enter a valid Forecast ID`
   - If actual prices missing: `⚠️ Please provide the realised price sequence`
